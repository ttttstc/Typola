...
- 修复 AI 工作台 AskUserQuestion 未回答就继续执行的问题：Claude headless 输入链路切到 OpenDesign 同款 `--input-format stream-json`，等待 `stop_reason=tool_use` 时保留 stdin，并在用户提交后写回 `tool_result`；同时所有非问答工具调用默认折叠。
