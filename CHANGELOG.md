...
- 修复 AI 工作台底部输入区被挤变形：发送按钮没有 `flex: 0 0 auto` / `nowrap`，模型名（如 `claude-opus-4-8[1M][1m]`）又不会收缩，两者争宽时「发送」被压成竖排两行；且按钮字号被全局规则刷回根字号（用户调到 14px 时比同排 12px 的模型标签还大）。现发送按钮不参与收缩、保持单行、字号归到 12.5px，模型名改为可收缩省略；面板标题「AI 工作台」也不再折行。
