...
- 修复会话下拉列表参差不齐：头部布局用的宽泛选择器 `.conversation-header div` 把下拉 `<div>` 一并命中，塞进了 `align-items:center` 与 `gap:8px`——导致每行按自身内容宽度居中收缩（标题长短不同 → 每行长度都不一样）、行距也被撑大。该规则改为只匹配头部直接子元素（`.conversation-header > div`），下拉恢复撑满行宽（实测 4 行均为 218×40、左右边界一致）；同时把下拉宽度钳到容器内（`min(300px, 100%)`）并让 pill 容器占满剩余宽度，列表不再溢出左栏盖住编辑区。
