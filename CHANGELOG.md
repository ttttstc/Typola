...
- AI 制品体验改版（定位：制品是会话绑定的临时原料，显式动作才升格为工作区资产）：① 生成制品后自动滑出右侧制品面板（新设置项「生成制品后自动打开面板」，默认开，可关）；② 工具栏「AI 产物」按钮新增未读角标——打开制品面板即视为已读，跨重启保持；③ 制品标题语义化——新制品从内容推导标题（HTML 取 `<title>`/首个 `<h1>`，Markdown 取首个一级标题，40 字截断），替代裸文件名；④ 「归档」更名「存为文件」并前置为主操作，点击弹命名输入框（默认取语义标题，可改；非法字符清洗、扩展名强制保留、重名自动去重），归档后卡片保留并标记「已归档」，点击打开工作区新路径（同时修复归档后 manifest 主文件路径悬空）——制品中心卡片、chips 面板与会话内生成后 toast 三个入口统一走该命名链路，不再有绕过命名、留下悬空主文件路径的旧路径；卡片新增「插入文档」按钮（markdown/检视版复用「合并到当前文档」分段 diff 链路），已归档卡片隐藏存为文件/插入文档/删除（其主文件已移出 `.typola-output`，删除命令必然被 Rust 拒绝）；⑤ 关闭会话即清理该会话的临时制品目录（conv-N）：有未归档制品或覆盖备份时先弹确认（明示数量与「清理后撤销覆盖不可用」），已存为文件/已插入文档的内容不受影响，运行中的会话禁止关闭防写盘竞态——此前 conv-N 目录永久残留。

- 制品改版检视意见修复：① 「存为文件」的成功判定不再依赖自动打开——`onOpenPath` 改为 best-effort，避免打开失败就跳过 archived manifest 写回、留下 primaryFile 悬空的卡片；manifest 写回失败会显式提示（文件其实已保存成功）。② 关闭会话时清理失败改为**取消关闭**并提示，不再留下「会话已从 UI 消失、conv-N 却仍在磁盘」的 orphan 目录。③ 生成中与等待用户回答（`waitingForUser`）的会话一并禁止关闭，避免与 AI 进程写盘竞态。④ 清理成功后按会话目录前缀清掉 chips，不再残留指向已删文件的死路径。⑤ 「真实制品候选」判定前移到 watcher 写入处，并与 chips/manifest 链路共用同一 predicate——conv-N 目录不再被当 primaryFile 写出伪 manifest；watcher 的 remove 事件改为删除路径（此前一律 set，被删文件会永远留在 chips 里）。⑥ 未读角标水位在制品总数缩小时同步下调，修掉删除/清理后长期漏报。⑦ 自动打开改挂「新出现的制品路径」，watcher 兜底发现的制品也会触发（此前只有 provider 的 `artifact_file` 事件会）；角标因此改为「关闭自动打开」时的兜底提示。⑧ legacy 制品归档时补建 manifest，不再静默跳过导致卡片直接消失。

- 修复制品 watcher 兜底链路静默失效（exe 场景实测暴露）：① 监听事件路径过滤对绝对路径整体判 ignore，监听根本身是点开头目录（`.typola-output`）时根下所有事件被全灭——现 ignore 规则只作用于根的相对路径部分（工作区树根路径含 `.config` 等点开头组件时文件树监听同样受益）；② Windows 下 `canonicalize` 返回 `\\?\` verbatim 前缀路径，前端前缀匹配全部落空——现统一剥掉该前缀。③ 链路打通后暴露的非产物噪声：会话目录 `conv-N`（mkdir 事件）与 manifest `artifact.json` 此前会作为「产物」出现在 chips 里，可被打开/归档/删除（归档目录会把整个会话目录 move 进工作区）——chips 现只收「有扩展名的文件」且排除 manifest。两处理化后，「AI 写文件 → watcher → manifest → chips/制品中心」的 provider 无关兜底链路首次真正生效，并新增 exe 制品套件（watcher 落盘推导标题、chips/toast 归档弹命名框、非产物不进 chips、未读角标、存为文件命名与已归档留痕）。

- 会话下拉（ConversationPill）排版修复：长标题不再挤掉重命名/关闭按钮（flex 标题缺 `min-width:0` 导致 min-content 撑破固定宽度）；running 圆点与关闭按钮改为常驻占位渲染，逐行内容区宽度一致；条目新增消息数次级信息；圆角统一 8px 设计 token。会话首条消息自动命名放宽到全部默认标题（「自由对话」与「Claude/OpenCode 对话」均生效，此前仅前者触发，手动改名不覆盖）。

- 场景模板 skill 卡片：未安装项的「让 XX 安装」动作从右上角状态列移到**卡片右下角**（跨两列、右对齐），不再和「未安装」徽标挤在同一行；并去掉它被 define-color 全局按钮规则（`#root :is(button, …)`）刷上的填充底色——该文件里原有的豁免选择器层级不够、一直是失效的，现在改用带 `#root` 的更高层级选择器，动作与卡片其它文字同一配色（纯文字 + 下划线，hover 提到主文字色）。

- 修复 AI 工作台输入区与计数徽标：① 发送按钮没有 `flex: 0 0 auto` / `nowrap`，模型名（如 `claude-opus-4-8[1M][1m]`）又不会收缩，两者争宽时「发送」被压成竖排两行；且按钮字号被全局 `.app-layout button { font-size: inherit }` 刷回根字号（14px），比同排 12px 的模型标签还大。现输入区改为两行——模型名独占首行，`+` / 工作目录 / Provider 与发送按钮排第二行、按钮靠右；实测发送按钮 67×28 单行且始终落在面板内；面板标题「AI 工作台」加 `nowrap` 不再折行。② 会话计数徽标上的文字色来自 `--text-on-accent`——该变量此前**只有引用、没有定义**（无效声明），又被高对比纪律 `body :where(*) { color: #000 !important }` 强制成黑字，深色 accent 底上就是一坨看不出数字的黑点。现补上 `--text-on-accent`（取 `--theme-paper`，暗色主题下自适应）并把 accent 实底徽标（会话计数、制品未读角标）加入 `high-contrast.css` 的反色表面豁免，与 tooltip / conversation toast 同一处理。

- 修复会话下拉列表参差不齐：头部布局用的宽泛选择器 `.conversation-header div` 把下拉 `<div>` 一并命中，塞进了 `align-items:center` 与 `gap:8px`——导致每行按自身内容宽度居中收缩（标题长短不同 → 每行长度都不一样）、行距也被撑大。该规则改为只匹配头部直接子元素（`.conversation-header > div`），下拉恢复撑满行宽（实测 4 行均为 349px、左右边界一致）；同时把下拉的定位参照从 pill 容器改为整个会话头部，宽度与左栏对齐（实测 359px，与左栏 360px 一致），既不溢出左栏盖住编辑区，也不再随 pill 宽度收窄。

- 右栏开关改为常驻：此前「折叠右栏」按钮只在右栏已打开时出现，阅读模式（右栏为 `none`）下根本没有入口，看起来像功能缺失。现按钮常驻工具栏最右端——无右栏时显示「打开右栏」，点击打开上次用过的右栏（首次默认 Word 预览）；已打开时折叠/展开。右栏模式的记忆随会话，切换文档模式（阅读/心流/检视）后仍能一键还原上次的右栏。

- 工具栏瘦身：13 个 Markdown 格式按钮全部平铺时，工具栏内容约需 1531px，而默认窗口仅 980px 且无最小宽度限制——溢出部分一律从右端被裁，设置、文档模式切换（阅读/心流/检视）、右栏开关在窗口较窄时会被整片切掉且无任何提示。现保留 6 个高频格式（加粗 / 斜体 / 链接 / 代码块 / 无序列表 / 有序列表）平铺，其余 7 个低频项（删除线 / 高亮 / 公式块 / 引用块 / 任务列表 / 分隔线 / 格式刷）收进新的「更多格式」下拉；工具栏最小宽度 1531px → 1315px，1366/1440/1920 窗口下右端控件完整可见。下拉复用既有 `ToolbarSplitMenu`（浮动定位 + 键盘可达 + `FloatingPortal`），与打开/保存/插入三处下拉同一套样式与交互。
- 大纲面板与场景模板 skill 列表视觉梳理：① 大纲此前 `.toc-h*` 只有类名没有样式，各级标题同为 13px、层级只能靠缩进表达——现按 heading level 给出字号/字重/颜色三重区分，选中态从通栏灰色硬块改为内缩圆角中性底；头部标题 10.5px → 11.5px 加粗，pin/关闭按钮 22px → 24px、圆角 3px → 6px。② skill 卡片此前被 `min-height: 112px` + `grid-template-rows: … minmax(0,1fr)` 撑出大片空白、把「产物/来源」顶到卡片底部——现按内容自然收拢；skill ID（`/markdown`）与标题同字号且 ID 更抢眼——现 ID 降为 11px muted、标题升为 13.5px/600；「已安装」徽标原用 paper 底叠在 control-bg 卡片上几乎不可见——现改同族中性底 + 边框；右侧动作列改顶对齐不再孤立于卡片中部。两处圆角统一到 8px、过渡改用主题 motion token。选中态与徽标的底色一律取自面板/卡片同族的中性阶（`--theme-surface-muted` / `--theme-control-active-bg`），不用 accent 染色——默认 `define-color` 配色下 `--theme-accent-soft` 走 neutralDark 族、而面板底色走 baseLight 族（按用户基色染色），两者不同族会显出「两个颜色」。③ 修复大纲与 skill 面板内**按钮字号全部失效**：文件后段的全局 `.app-layout button { font-size: inherit }` 与组件规则同特异性但更靠后，把 `.skill-hub-*` 与大纲按钮的字号一律刷回根字号（用户调到 14px 时「让 Claude 安装」比同排徽标 10.5px、卡片标题 13.5px 还大，整排也对不齐；大纲 h4 及更深反而比 h3 大）——相关规则改用父类提级，并让「让 Claude 安装」与同排「未安装」徽标同字号同高度（11px / 21px）。
...
- 修复 PR #282 检视意见：新增预览扩展统一受 Markdown 语法树代码范围约束，特殊图片路径在光标进入时恢复原始 Markdown，硬换行不误处理转义反斜杠；补充架构事实、2.0.9 版本同步和 atomic 链接交互回归。当前直接 exe 仍有 P0-11/P1-11 两个链接点击终态待继续处理，未以 source 保留替代真实打开验证。

- 修复微信/HTML 预览表格无可见样式：为所有公开 HTML 导出预设补齐共享表格宽度、边框、单元格间距和表头样式；Markdown 阅读预览与 Word 纸张预览的表格转换、分页和样式链路核对无回归。
...
- 修复 PR #268 检视意见（5 项）：① `sanitizeService` 的 `<style>` 放行收窄到 SVG 范围——清洗后显式移除不位于 `svg` 子树内的 `<style>` 节点，普通 Markdown/HTML 的 `<style>` 注入不再因 Mermaid 兼容改动存活（SVG 内嵌套层级的 style 照常保留）；② 跨文档视图状态隔离——A/B 两文件内容完全一致时切换不再被"正文相同"提前返回吞掉，path 先更新、异步 source 后到的分阶段切换也正确走文档切换分支，选区/滚动保存恢复不丢；③ 列表 Tab/Shift-Tab 多选边界——`selection.to` 排他边界落在下一行行首时不再把下一行误算进覆盖范围，多选列表项（选区结束于下一普通行行首）缩进不再失效；④ AI 改稿意见闭环改逐意见确认——候选稿整体应用成功后，只有锚点原文在候选稿中不再原样出现（该段确被改动）的意见才标记「已应用」，AI 跳过、锚点未动的意见保持待处理，消除批量关闭的假闭环（新增 `resolveAppliedCommentIds`）；⑤ 补同步 `pnpm-lock.yaml` 的 turndown 相关条目（CI `pnpm install --frozen-lockfile` 不再失败）。均附回归测试。

- 修复右键菜单系列功能 bug 并与 Typora 行为对齐：① 右键命令作用于旧选区而非右键落点——右键时若落点不在当前选区内先把光标移到落点（段落/格式/编辑语言/编辑链接等命令作用于右键所在块），落点在选区内则保留选区（剪切/复制仍可用）；② "编辑语言"不再要求选区精确覆盖整个代码块，改为按语法树 `FencedCode` 节点（覆盖光标位置）定位代码块，语法树不可用时回退 fence 行扫描，右键块内任意行都能弹语言编辑弹窗，且保存时只重写开 fence 行、块体不动；③ mermaid 图右键"复制为 SVG"接线修复——右键命中 `.typola-cm6-mermaid` widget 且内部已渲染 svg 时显示该菜单项，点击将 SVG 源码文本写入剪贴板（错误态/渲染中不显示）；④ 右键+选区时 AI 浮条与菜单叠加——拖选追踪只响应左键 mousedown，右键不再触发浮条浮现；⑤ 菜单"粘贴"与键盘 Ctrl+V 行为对齐——新增 `applyClipboardData` 智能链路（表格 TSV/CSV/HTML → 结构化 HTML 转 Markdown → 纯文本回退），键盘 onPaste 与菜单"粘贴"共用，菜单粘贴优先 `clipboard.read()` 取 HTML+纯文本，不可用时回退 `readText`。新增右键菜单回归测试（EditorPane 落点/mermaid/粘贴链路、cm6FormatService 标题升降级与代码块定位、EditorContextMenu 新菜单项）与 e2e（右键段落→H2 作用于落点行、右键代码块→编辑语言）。

- "段落"子菜单对齐 Typora 高频项：新增"提升标题等级 (Ctrl+=)"（H2-H6 升一级、正文变 H2、H1 保持）、"降低标题等级 (Ctrl+-)"（H1 回正文、H2-H5 降一级、H6/正文保持）、"引用"、"无序列表"、"有序列表"入口；`createMarkdownExtensions` 的 onFormat keymap 追加 `Mod-=` / `Mod--` 两条绑定。

- 修复检视模式「应用全部检视意见 / AI 改稿」链路系列问题：① 应用后意见不闭环——AI 改稿应用成功后，本次发送的检视意见自动标记「已应用」（检视列表显示徽标、不再计入待处理数），不再把锚点已失效的意见原样塞进下一轮改稿 prompt 空转；已应用意见保留历史、导出检视版时标注「已应用」且元数据往返不丢。② 长锚点不再截断丢信息——AI 改稿 prompt 中锚点原文截断阈值从 160 提升至 500 字符，仍超长时显式提示 AI 结合前缀在文稿中定位整段。③ 跨文档不再挂起——AI 改稿运行中切换文档后，改稿完成时不再永久挂起（全局「AI 改稿」按钮不再永久禁用），改为一次性提示切回原文档查看，候选稿可从改稿历史打开。④ 应用按钮双击并发——useDiffReview 增加进行中守卫，历史快照落盘期间重复点击只执行一次应用，失败后守卫自动复位可重试。⑤ 人工检视意见锚点补 prefixHint（选区前 24 字符），重复文本多处出现时定位不再撞错位置。⑥ 导出检视版同段多意见顺序修复——同一落点插入多条意见时按意见列表顺序输出，不再反转。新增意见闭环、并发守卫、同段顺序、prefixHint 消歧回归测试。
...
- Unix 下原子保存 rename 后对父目录执行 fsync，掉电场景下目录项可靠持久化（Windows 已由 MOVEFILE\_WRITE\_THROUGH 覆盖）。
...
- “忽略此版本”现在持久化版本号；新版本恢复提示，手动检查可重新显示已忽略版本。

- 未保存文档对话框增加焦点 trap、背景 inert、焦点恢复和多文档“全部保存”；“打开文件夹”只建立工作区，不再批量打开目录文档。

- 原子保存替换已有文档时保留 Unix 文件权限，避免私有文档因临时文件默认权限变得可读。

- 发现新版本的更新卡片新增“忽略此版本”按钮，点击后立即关闭本次提示。

- 修复关闭当前标签左侧的文件后，活动标签背景指示器仍停留在已关闭文件旧位置的问题。

- 修复打开现有 Markdown 后正文已载入但仍显示“未命名”、未打开文件、错误标签或已修改的问题；打开、切换和关闭标签时会先同步当前文档身份，表格插件的首次自动排版不再触发未保存星号，切换到干净文件时同步清除旧的保存状态提示。

- 关闭窗口遇到多个未保存文档时改为逐篇确认，显示当前文件与处理进度；每篇可保存、不保存或取消关闭，也可一次选择“全部不保存”。
...
- 工具栏新增「打开文件夹」按钮(Cmd+Shift+O),选夹后走新增 Tauri cmd `read_first_level_openable` 仅列一层 md/html/docx(不递归,跳过隐藏文件与节点\_modules/dist/target/.git),批量入 tab(last active),单文件打开失败不阻塞其他(#170);新增 keys `toolbarOpenFolderTitle / toolbarOpenFolderLabel` 中英日三译;fileService 加 `openFolder` 函数,useFileTabs 加 `handleOpenFolder` 回调。
