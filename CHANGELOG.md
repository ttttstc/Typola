...
- 补齐 Issue #280 暴露的 Markdown 写作视图边界：原始 HTML 的注释、脚本和模板标签不再泄漏为可见源码，`==高亮==` 在写作视图保留 mark 语义，光标位于文档末尾时上标仍能渲染；兼容中文与空格图片路径，并校正 SVG、链接和围栏代码块的 exe 验证断言。扩展 exe 套件现为 111/111 通过。
