...
- 优化 Define 自定义配色下设置页的明暗层级：导航、页签、预设选择与预览容器统一采用当前选色色相的低饱和阶梯，不再被通用控件样式强制成大片纯白；Word 预览仅保留轻微染色的纸张层，避免突兀。
- 优化 AI 工作台 Skill 卡片：未安装项不再降低整卡颜色，安装操作改为不占位的透明文字按钮；窄侧栏不会挤压或折行越界。左上角主收起按钮在 AI 工作台中现在一次点击即可关闭。工具栏“打开文件 / 打开文件夹”图标已校正；Windows NSIS 安装包为 Markdown、HTML 和 Word 文档注册“用 Typola 打开”右键命令。
