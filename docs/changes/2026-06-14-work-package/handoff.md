...
2. codex 会**先做 R1/R2/R3 spike** 并暂停等你确认（产物：`docs/changes/2026-06-14-work-package/spike-notes.md`）。
...
当前主分支 `main` 已含：文件树、多 tab、未保存三按钮对话框、WYSIWYG 代码块焦点态、
右键菜单、自定义 tooltip、新建文件、编辑器→预览同步滚动、配色统一。
...
# 工作目录
仓库根 D:\暂存\Typola。已是干净状态，工作分支随便起一个新分支（例如 codex/flow-mode-mvp）从 main 拉。

# 第一步（必须，且只做这一步就暂停）
按 spec §13 做 R1/R2/R3 spike：
...
- R2: 在 claude TUI 里 writeTerminal(termId, '\x1b[200~hello\x1b[201~\r')
  → 验证 bracketed paste + 末尾 \r 把多行作为一次输入提交。失败则记录 fallback。
- R3: writeTerminal(termId, '\x1b[Z')
  → 验证 TUI 底部模式从 normal → acceptEdits → plan 循环切换。

把三条结论写到 docs/changes/2026-06-14-work-package/spike-notes.md，
包括「OK / fallback 内容 / 阻断」三档结论 + 具体观测到的现象。
然后**停下来报告给我，等我确认再继续。不要直接进入 §4–§9 实施。**

Spike 时**不需要写正式代码**，可以临时在 TerminalPanel 加几行调试调用，验证完撤销。
spike-notes.md 是唯一要提交的产物（commit 信息 `spike: 心流模式 R1/R2/R3 验证`）。

# Spike 通过后才进入的步骤（每完成一节 commit + 报告 + 等我确认再进下一节）
按 spec 顺序：
§4 心流模式布局编排（flowMode state、Toolbar 开关、布局一键点亮、flowRightTab）
§5 场景启动器（FlowScenario 类型、注册表 JSON、ScenarioPanel、agentBridge.run）
§7 件1 选区注入（EditorCommandHandle.getSelection、SelectionSendButton、agentBridge）
§6 闭环输出（6.1 自动 reload + 6.2 workspace watch + agentChangedPaths + 本次产物列表）
§8 agent 终端启动 + 权限切换（TerminalPanelHandle、Shift+Tab 切 acceptEdits）

实施顺序可以是 §4 → §8（先把骨架 + 终端起来）→ §7（最小可用的注入）→ §5（场景卡）→ §6（闭环输出）。
按你判断的依赖关系来，前提是**每一节自己内部能独立工作**（可被验收）。
...
4. **不擅自加功能**：spec §1 「非目标」列出的东西全部不做（特别注意：
   不做 git diff 视图、不做跟随模式、不做多引擎、不做场景图形编辑器、
   件1 选区注入保持纯注入无预设/不自动回车）。
5. **遇到 spec 不清楚或冲突**：停下来问我，不要按猜测继续。
6. **权限切换实现遵守 spec §8**：Shift+Tab 在会话内切换，**不重起 agent 会话**。
   spec §8.1 已明确单 session 是硬要求。
...
`feat: 心流模式 MVP（场景启动器 + 选区注入 + 闭环输出）`，body 引用本 spec
和 spike-notes 的链接。

# 开始
请先读完三份必读文档，然后开始做 R1/R2/R3 spike 并把结论写到 spike-notes.md。
做完 spike 后停下来等我确认。
```
