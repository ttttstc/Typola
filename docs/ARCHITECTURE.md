...
- `src/components/editor/cm6/Cm6MarkdownEditorPane.tsx` is the experimental Phase 1 CM6 Markdown editor entry. It currently reuses the proven source-mode CodeMirror command path behind the local `typola.editorEngine=cm6` feature flag, while `src/components/editor/cm6/createMarkdownExtensions.ts` centralizes CM6 Markdown extension construction for the upcoming live-preview migration.
