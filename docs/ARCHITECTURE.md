...
- Frontend parsing is isolated in `src/services/agent/claudeStream.ts` and related guard/diagnostic helpers. `useAgentSession` listens to `agent-stdout` and `agent-exit`, then accumulates user messages, assistant text, thinking deltas, tool cards, usage summaries, and human-readable error diagnostics for `ConversationPanel`.
