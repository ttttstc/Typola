...
- Visible context chips also produce provider reference paths on every send. Claude receives these paths through prompt text and allowed-directory arguments. OpenCode does not receive synthetic reference text in the prompt; it receives repeated `--file <path>` arguments each turn, because its prompt is passed as a command argument on Windows and first-turn multiline reference text can be misinterpreted by the `.cmd` launcher. OpenCode runs with its process cwd in the conversation artifact directory, but its `--dir` project directory points at the effective workspace root so file discovery matches the file tree/current-document directory instead of the empty `.typola-output` conversation folder.
...
- SkillHub system templates are scoped by AI Provider before rendering install actions. Existing built-in skill templates are Claude-only, so OpenCode conversations do not show Claude skill install prompts; user-added scene skills remain available as local slash commands.
