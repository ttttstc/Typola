...
- `src/components/editor/cm6/Cm6MarkdownEditorPane.tsx` is the experimental CM6 Markdown editor entry behind the local `typola.editorEngine=cm6` feature flag. It reuses the proven source-mode CodeMirror command path while `src/components/editor/cm6/createMarkdownExtensions.ts` centralizes CM6 Markdown extension construction. In live-preview mode it enables atomic-editor task list/table/image widgets plus Typola-owned KaTeX and Mermaid widgets; source mode keeps live preview disabled.
