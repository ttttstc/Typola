...
### 15.2 模块 A：skill 选择 + 运行期多会话（差异化招牌）
...
- **A3 skill 库 UI**：放**右侧栏上段**（布局总览：右 = 场景 + 产物）；点 skill → **新建一个会话**（见 A4），标题设 skill 名，输入框预填 `/skill-name ` 等用户补任务内容再发
...
| 6 | skillHubService：读 skill 库 + skill-hub.json + 分类管理 + 迁移 flow-scenarios | `services/skillHubService.ts` | — |
| 7 | SkillHub UI（右侧栏分类 + skill 列表 + 设置导入入口） | `components/SkillHub.tsx` | 6 |
| 8 | 点 skill → 新建会话（标题=skill 名 + 预填 `/skill-name`） | `AppLayout` + 会话 store + SkillHub | 4,7 |
...
4. ✅ 点 skill → **新建独立会话**、标题=skill 名、预填 `/skill-name`、独立 claude session 上下文
5. ✅ 会话切换器是 **pill + 下拉**（非 tab，形态区别于编辑器文件 tab）；能 新建/切换/关闭 多个会话，切回上下文不丢（同一运行期内）；下拉无 重命名/搜索/归档
6. ✅ 旧 ScenarioPanel/flowScenario 已移除，无残留引用
7. ✅ typecheck / test / cargo / build 全过
