...
- **状态**：M1 端到端通过（2026-06-18，minimax 验证 headless 调用链全通）；M2 范围已与用户确认 = **产物回流 + skill 选择**（选区注入推 M3）
...
**B2. skill 落盘文件 → 工作区**
- skill 自己写文件（如 baoyu-slide-deck 生成 html）→ workspace watcher 捕获 → 产物浮窗（复用 `ArtifactPreview`）/可点开
- **关键接线**：watcher 监听 **AI 工作台 cwd**（M1 已把它和文件树 workspaceRoot 解耦），不是文件树根
- 复用现成：watcher + ArtifactPreview + selfWriteFilter（1500ms 自写过滤）

### 15.2 模块 A：skill 选择（差异化招牌）

让"调 skill"从手敲 `/skill-name` 变成点选，立住"文档 Skill OS"身份。

- **A1 skill 库读取**：从 headless `init` 事件拿 skill 列表（name）；description 读 `~/.claude/skills/<id>/SKILL.md` frontmatter 补充
- **A2 分类/收藏**：`skill-hub.json`（用户建分类，每类挂 skill 引用，**只存名不拷内容**）；设置入口浏览全部 skill → 加入分类
- **A3 skill 库 UI**：放**右侧栏上段**（spec 布局总览：右 = 场景 + 产物）；点 skill → 左侧 AI 工作台当前对话拼 `/skill-name` 作首轮（或填入输入框让用户补 prompt 再发）
...
| 3 | 产物 watcher 接 AI 工作台 cwd → ArtifactPreview 浮窗 | `AppLayout.tsx` / `workspaceWatchService.ts` | — |
| 4 | skillHubService：读 skill 库 + skill-hub.json + 分类管理 + 迁移 flow-scenarios | `services/skillHubService.ts` | — |
| 5 | SkillHub UI（右侧栏分类 + skill 列表 + 设置导入入口） | `components/SkillHub.tsx` | 4 |
| 6 | 点 skill → 拼 `/skill-name` 进 AI 工作台对话 | `AppLayout` + `ConversationPanel` | 5 |
| 7 | 砍旧 ScenarioPanel / flowScenario*（被 SkillHub 取代） | — | 6 |
| 8 | i18n + CSS + 验证（typecheck/test/cargo/build） | — | 全部 |

### 15.4 M2 验收

1. ✅ 对话里的回复/代码块能一键 复制 / 插入光标 / 替换选区 进文档
2. ✅ skill 生成的文件自动出现在右侧产物浮窗、可点开
3. ✅ skill 库按分类显示，能从全部 skill 导入收藏
4. ✅ 点 skill → AI 工作台开对话、自动拼 `/skill-name`
5. ✅ 旧 ScenarioPanel/flowScenario 已移除，无残留引用
6. ✅ typecheck / test / cargo / build 全过

### 15.5 不在 M2 范围（聚焦，推 M3+）

❌ 选区注入（选中→AI，M3）· AI 编辑原语 slash/streaming（M3）· **会话持久化**（M2 仍内存 Map；2026-06-18 评估：跨重启保留值得做，但当前规模 JSON 够、SQLite 过度，整体推 M3+，避免分散 M2 聚焦）· 路径可点击 / 态势感知 / 项目记忆 / 落盘脉冲（M3 按痛感）· Git diff / AutoGit / Skills 透视 / Agent 用量 / 多 agent（文档场景错配或重型，不主动做）
