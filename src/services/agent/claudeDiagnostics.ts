 ...
 * Original: apps/daemon/src/claude-diagnostics.ts
 * Copyright the Open Design authors. Licensed under Apache License 2.0.
 * Modifications: ported to Typola import paths and browser-safe basename handling.
 */
...
function selectedClaudeCompatibleRuntime(input: ClaudeCliDiagnosticInput): 'claude' | 'openclaude' {
  ...
  const normalizedPath = input.resolvedBin.trim().replace(/\\/g, '/');
