export type EditorCommandHandle = {
  ...
  // opts.preserveFocus:true 时不抢焦点回编辑器 —— 搜索导航必须传 true,
  //   否则用户在 FindReplacePanel 输入时焦点会被抢回,后续按键打进文档。
  revealRange: (from: number, to: number, opts?: { text?: string; preserveFocus?: boolean }) => void;
