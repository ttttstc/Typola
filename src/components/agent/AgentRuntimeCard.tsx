  ...
  return (
    <section className={`agent-runtime-card ${active ? 'active' : ''}`} aria-label={`${runtime.label} 设置`}>
      <header className="agent-runtime-card-header">
        <button
          type="button"
          className="agent-runtime-summary"
          aria-expanded={expanded}
          aria-controls={detailsId}
          onClick={() => setExpanded((current) => !current)}
        >
          <span className="agent-runtime-summary-copy">
            <span className="agent-runtime-summary-title">
              <strong>{runtime.label}</strong>
              <span className={`agent-runtime-status ${statusTone}`}>{statusLabel}</span>
            </span>
            {runtime.description && <span className="settings-desc agent-runtime-desc">{runtime.description}</span>}
          </span>
          <ChevronDown
            className={`agent-runtime-chevron ${expanded ? 'expanded' : ''}`}
            size={16}
            strokeWidth={1.6}
            aria-hidden="true"
          />
      ...
      {expanded && (
        <div id={detailsId} className="agent-runtime-details">
