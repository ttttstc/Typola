...
维护信息：最近核对日期为 2026-09-21；本索引与同级 15 个条目、27 个直接 exe 场景均已从当前源码入口核对。`npm run verify:exe-core` 是核心套件（18 action pass / 7 skipped，run `2026-09-21T00-40-32-476Z-34308`；同一 Markdown 表格已在 Word 与 HTML 预览中实跑确认）；`npm run verify:exe-core:extended` 是扩展套件（**111 action: 111 passed / 0 failed / 12 skipped**，run `2026-09-21T00-41-03-819Z-51060`），其中 Issue #280 的 P0/P1/P2 渲染边界场景已全部通过。完整非 AI 实跑汇总：`npm run verify:exe-core:all`（`run-all-non-ai.mjs`，汇总文件 `.nimo/verification/evidence/_summary-non-ai.json`）。当前两套件没有失败项；12 个 extended skip 与 7 个 core skip 保留为真实外部前置缺口，未把原生对话框、发布物或 AI 认证路径冒充为通过。直接 exe 套件证据保存在 [`evidence/`](../evidence/) 下按运行生成的 `*-exe-core-suite` 与 `*-exe-core-extended` 目录，两套件进程、profile、runtime 夹具和 CDP 均已清理。
