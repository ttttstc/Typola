...
| 改坏了想撤回 AI 的修改 | `Ctrl+Z` 撤销 N 次,顺手撤销自己的手改 | `Ctrl+Z` 智能区分:先撤销手改,再撤销 AI,互不冲突 |
...
- 查找替换(`Cmd/Ctrl+F` / `Cmd/Ctrl+H`,大小写 / 全词 / 正则)+ 快速打开(`Cmd/Ctrl+P`)
...
下载对应芯片架构的 `.dmg`,打开后把 `Typola.app` 拖入 Applications。Portable zip 解压即跑。首次运行如遇系统安全提示,在「隐私与安全」中允许打开。
