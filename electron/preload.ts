...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  showSaveDialog: (options: { defaultPath?: string; filters?: { name: string; extensions: string[] }[] }) =>
    ipcRenderer.invoke('show_save_dialog', options),
  saveImage: (workspaceRoot: string, data: number[], ext: string) =>
    ipcRenderer.invoke('save_image', workspaceRoot, data, ext),
  ...
  onFileChanged: (callback: (data: { path: string }) => void) => {
    const handler = (_: any, data: { path: string }) => callback(data);
